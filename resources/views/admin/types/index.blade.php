    ...
    <div class="m-subheader ">
        <div class="d-flex align-items-center">
            <div class="mr-auto">
                ...
                {!! Breadcrumbs::generate(['Рабочий стол', 'Типы типов'], 'metronic') !!}
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>
                        Вы действительно желаете удалить тип?
                    </p>
                </div>
            ...
            var e = function () {
                var e = JSON.parse('<?php echo $units; ?>')
                    ...
                    columns: [{
                        ...
                        overflow: "visible",
                        template: function (e) {
                            var onclick = 'return $(\'#m_modal .attr__delete\').attr(\'href\', \'/admin/types/delete?id=' + e.id + '\')';
                            return '<div class="dropdown ' + (e.getDatatable().getPageSize() - e.getIndex() <= 4 ? "dropup" : "") + '">' +
                                '<a href="#" class="btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" data-toggle="dropdown">' +
                                '<i class="la la-ellipsis-h"></i>' +
                                '</a><div class="dropdown-menu dropdown-menu-right">' +
                                '<a class="dropdown-item" href="/admin/types/' + e.id + '/edit"><i class="la la-edit"></i> Редактировать' +
                                '<a class="dropdown-item" href="#" data-target="#m_modal" data-toggle="modal" ' +
                                'onclick="' + onclick + '">' +
                                '<i class="la la-trash"></i> Удалить' +
                                '</a></div></div>' +
                                '<a href="/admin/types/' + e.id + '/edit" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="View ">' +
