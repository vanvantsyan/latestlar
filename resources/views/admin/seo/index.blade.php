        ...
        <div class="d-flex align-items-center">
            <div class="mr-auto">
                <h3 class="m-subheader__title m-subheader__title--separator">
                    Сео для генерируемых страниц
                </h3>
                {!! Breadcrumbs::generate(['Рабочий стол', 'Сео'], 'metronic') !!}
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Удаление сео</h5>
                ...
                <div class="modal-body">
                    <p>Вы действительно желаете удалить сео?</p>
                </div>
                ...
                var e = JSON.parse('<?php echo $units; ?>')
                    ...
                    columns: [{
                    ...
                    },
                        {
                            ...
                            overflow: "visible",
                            template: function (e) {
                                var onclick = 'return $(\'#m_modal .attr__delete\').attr(\'href\', \'/admin/seo/delete?id=' + e.id + '\')';
                                return '<div class="dropdown ' + (e.getDatatable().getPageSize() - e.getIndex() <= 4 ? "dropup" : "") + '">' +
                                    '<a href="#" class="btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" data-toggle="dropdown">' +
                                    '<i class="la la-ellipsis-h"></i>' +
                                    '</a><div class="dropdown-menu dropdown-menu-right">' +
                                    '<a class="dropdown-item" href="/admin/seo/' + e.id + '/edit"><i class="la la-edit"></i> Редактировать' +
                                    '<a class="dropdown-item" href="#" data-target="#m_modal" data-toggle="modal" ' +
                                    'onclick="' + onclick + '">' +
                                    '<i class="la la-trash"></i> Удалить' +
                                    '</a></div></div>' +
                                    '<a href="/admin/seo/' + e.id + '/edit" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="View ">' +
